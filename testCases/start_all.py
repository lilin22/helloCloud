    ...
    def assign_by_users(self,cmd,allure_dir,modules_run_caseNos):
        ...
        for pn in user_modules.keys():
            ...
            if modules != []:
                ...
                t = multiprocessing.Process(target=self.start_task,args=(pn,password,message,module_paths,modules_run_caseNos,cmd,allure_dir),daemon=True)
                process.append(t)
        return process

    @utils.catch_exception
    def start_task(self,mobile,password,message,module_paths,modules_run_caseNos,testcmd,allure_dir):
        # 测试代码
        ...
        for module_path in module_paths:
            ...
            cmd = [f'--mobile={mobile}',f'--password={password}',f'--message={message}',f'{path}',f'--alluredir={allure_dir}'] + testcmd
